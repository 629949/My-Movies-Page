...
function Anime() {
  ...
  const movieList = List.map((movie, index) => (
    ...
    <a href={movie.link} target="_blank" rel="noopener noreferrer" className=' flex w-1/3 h-12 rounded-lg justify-center bg-amber-500 text-black items-center font-bold'>
        {movie.Trailer}
      </a>
      </div>
  </div>
  ...
  return (
    <div className='flex flex-col bg-[#070F2B] text-white gap-10 w-full  place-content-center'>
     ...
     <div className='flex flex-col justify-center gap-10 w-3/4 ml-60 text-xl mt-10 '>{movieList}</div>
      </div>
      <Footer/>
    </div>
